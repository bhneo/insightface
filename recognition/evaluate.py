...
def train_net(args):
    ...
    pre_sym, arg_params, aux_params = mx.model.load_checkpoint(args.pretrained, args.pretrained_epoch)
    sym = get_symbol(pre_sym, args)
    ...
    model = mx.mod.Module(
        context       = mx.gpu(),
        symbol        = sym,
        fixed_param_names=arg_params.keys()
    )
    val_dataiter = None
    ...
    opt = optimizer.SGD(learning_rate=args.lr, momentum=args.mom, wd=args.wd, rescale_grad=1)
